...
MART_CONTRACTS: tuple[MartSpec, ...] = (
    ...
    MartSpec(
        ...
        provenance_scope="release_manifest",
        authorized_routes=("/utilization/procedures/search",),
    ),
    MartSpec(
        table="provider_drug_detail",
        transform_ids=("build_provider_drug_detail",),
        grain="one provider NPI by brand, generic drug, and data year",
        key_columns=("npi", "brand_name", "generic_name", "data_year"),
        upstream_tables=("raw_part_d_by_provider_and_drug", "core_providers"),
        source_ids=("cms_part_d_by_provider_and_drug",),
        required_columns=("npi", "brand_name", "generic_name", "data_year"),
        source_period_policy="release manifest; data_year is the Part D measurement year",
        provenance_scope="release_manifest",
        authorized_routes=("/utilization/drugs/search",),
    ),
    MartSpec(
        table="utilization_procedure_dictionary",
        transform_ids=("build_utilization_dictionaries",),
        grain="one HCPCS code and data year",
        key_columns=("hcpcs_code", "data_year"),
        upstream_tables=("provider_service_detail",),
        source_ids=("cms_physician_by_provider_and_service",),
        required_columns=(
            "hcpcs_code", "physician_count", "total_services", "total_payments", "data_year",
        ),
        source_period_policy="release manifest; data_year is the CMS measurement year",
        provenance_scope="release_manifest",
        kind="serving",
        authorized_routes=("/utilization/procedures/options",),
    ),
    MartSpec(
        table="utilization_drug_dictionary",
        transform_ids=("build_utilization_dictionaries",),
        grain="one brand and generic drug pair by data year",
        key_columns=("brand_name", "generic_name", "data_year"),
        upstream_tables=("provider_drug_detail",),
        source_ids=("cms_part_d_by_provider_and_drug",),
        required_columns=(
            "brand_name", "generic_name", "physician_count", "total_claims",
            "total_drug_cost", "data_year",
        ),
        source_period_policy="release manifest; data_year is the Part D measurement year",
        provenance_scope="release_manifest",
        kind="serving",
        authorized_routes=("/utilization/drugs/options",),
    ),
    MartSpec(
