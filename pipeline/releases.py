...
NPPES_SERVING_PRACTICE_CHANGED_TABLES = frozenset(
    {
...
def build_nppes_serving_practice_warehouse_release(
    *,
    data_root: Path,
    baseline_warehouse_release_id: str,
    backup_manifest_path: Path,
    ...
    """Build only the NPPES-primary serving tables on a verified baseline copy."""
    from .transform import build_serving_practice_nppes_tables

    _validate_targeted_build_resources(memory_limit_gb, threads)
    if data_year < 2000 or data_year > 2100:
        raise ReleaseError("NPPES serving-mart data year must be between 2000 and 2100")
    ...
    baseline_release = _find_release(document, baseline_warehouse_release_id)
    if baseline_release.validation_state != ValidationState.PASSED:
        raise ReleaseError("NPPES serving-mart baseline has not passed validation")
    if baseline_release.sha256 != baseline_sha:
        raise ReleaseError(
            "NPPES serving-mart backup SHA-256 does not match the named baseline release"
        )
    source_periods = baseline_release.validation_details.get("source_periods")
    if not isinstance(source_periods, dict):
        raise ReleaseError("NPPES serving-mart baseline lacks source-period provenance")
    contract_tables = (
        "serving_practice_nppes_provider_sites",
        "serving_practice_nppes_org_memberships",
    )
    ...
    if missing_period_sources:
        raise ReleaseError(
            "NPPES serving-mart baseline lacks required source periods: "
    ...
    if inherited_smoke_counts is not None and (
        ...
        raise ReleaseError(
            "NPPES serving-mart baseline has invalid smoke table-count evidence"
        )

    identity = hashlib.sha256(
        (
            "serving_practice_nppes_tables\0"
    ...
    with _exclusive_lock(data_root / "locks" / "build.lock"):
        ...
        try:
            ...
            try:
                ...
                connection.execute("BEGIN TRANSACTION")
                try:
                    table_counts = build_serving_practice_nppes_tables(
                        connection, data_year
                    )
                    contract_validation = validate_mart_contracts(
                        connection,
                ...
                release.validation_details = {
                    "release_scope": "targeted_additive",
                    "comparison_policy": "serving_practice_nppes_additive_v1",
                    "baseline_warehouse_release_id": baseline_warehouse_release_id,
                    "source_periods": dict(sorted(source_periods.items())),
                    "changed_tables": sorted(NPPES_SERVING_PRACTICE_CHANGED_TABLES),
...
def _comparison_policy(
    ...
    if (
        ...
        if declared_changed_tables != set(NPPES_SERVING_PRACTICE_CHANGED_TABLES):
            raise ReleaseError(
                "NPPES serving-mart release has an invalid changed-table allowlist"
            )
        if not isinstance(baseline_release_id, str) or not baseline_release_id:
            raise ReleaseError(
                "NPPES serving-mart release lacks its baseline release identity"
            )
        if not isinstance(evidence, dict) or set(evidence) != set(
            NPPES_SERVING_PRACTICE_CHANGED_TABLES
        ):
            raise ReleaseError(
                "NPPES serving-mart release lacks exact row-count evidence"
            )
        try:
            expected_counts = {
                table: int(count) for table, count in evidence.items()
            ...
            raise ReleaseError(
                "NPPES serving-mart row-count evidence is invalid"
            ) from error
        if any(count <= 0 for count in expected_counts.values()):
            raise ReleaseError(
                "NPPES serving-mart row-count evidence must be positive"
            )
        return (
            "serving_practice_nppes_additive_v1",
            NPPES_SERVING_PRACTICE_CHANGED_TABLES,
